...
namespace Mighty
{
    [System.Serializable] public class OnEnterGameStateStartedEvent : UnityEvent<string, string> { } // enteringGameState and exitingGameState
    [System.Serializable] public class OnEnterGameStateFinishedEvent : UnityEvent<string, string> { } // enteringGameState and exitingGameState

    [System.Serializable] public class OnExitGameStateStartedEvent : UnityEvent<string, string> { } // exitingGameState and enteringGameState
    [System.Serializable] public class OnExitGameStateFinishedEvent : UnityEvent<string, string> { } // exitingGameState and enteringGameState
    ...
    public abstract class MightyGameManager : MonoBehaviour
    {
        ...
        public abstract IEnumerator OnEnterGameState(string enteringGameState, string exitingGameState);

        // This is called on every game state exit (you decide to handle it or not)
        public abstract IEnumerator OnExitGameState(string exitingGameState, string enteringGameState);
    }
    ...
    public class MightyGameBrain : MonoBehaviour
    {
        ...
        [BoxGroup("Debug")] public bool displayMightyDebugInfo;
        ...
        IEnumerator TransitToNextGameStateInternal(string nextGameStateName)
        {
            if (currentGameStateName == NONE_STATE_NAME) // Initial transition to start state happening initializaion of the game
            {
                ...
                StartCoroutine(mightyGameManager.OnEnterGameState(enteringGameState.name, NONE_STATE_NAME));
            }
            else
            {
                if (currentGameState.HasExitGameState(nextGameStateName))
                {
                    ...
                    if (enteringGameState != null)
                    {
                        ...
                        yield return StartCoroutine(mightyGameManager.OnExitGameState(exitingGameState.name, enteringGameState.name));
                        ...
                        yield return StartCoroutine(mightyGameManager.OnEnterGameState(enteringGameState.name, exitingGameState.name));
