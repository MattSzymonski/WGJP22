...
{
    public static Collider[] ConeCastLayer(this Physics physics, Vector3 origin, float maxRadius, Vector3 direction, float maxDistance, float coneAngle, int layerMask)
    {
        ...
        if (sphereCastHits.Length > 0)
        {
            for (int i = 0; i < sphereCastHits.Length; i++)
            {
                ...
                {
                    coneCastHitList.Add(sphereCastHits[i].collider);
                }
            }
        }
